...
function createUmzug (flags, connection: ): Umzug {
  return new Umzug({



    storage: new UmzugPgStorage(),
...
async function _ensureFolder(dir: string) {
  try {
    return fsp.stat(dir)
  } catch {
...
export default async function mig(command, flags, progress) {
  flags = flags || {}
  progress = progress || function () {}

  const umzug = createUmzug(flags, knexInit(flags))
  ...
  const api = {
    ...
    rollback: async () => {
      ...
      const { batch: maxBatch } = maxBy(migrations, 'batch')
